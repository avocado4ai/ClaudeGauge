...
class ModelsScreen : public LcarsScreen {
public:
    const char* title() const override { return "TODAY MODELS"; }
    ...
    void onDraw(LcarsCanvas& spr, const LcarsFrame::Rect& c) override {
        ...
        _drawModels(spr, c, _state->usage);
    }

private:
    const AppState* _state = nullptr;

    void _drawModels(LcarsCanvas& spr, const LcarsFrame::Rect& c, const UsageData& usage) {
