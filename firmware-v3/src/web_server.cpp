...
<div class="footer">ClaudeGauge v2.0</div>
</div>
<script>
...
void ConfigWebServer::handleSaveSessionKey() {
    ...
    Serial.println("Session key saved");
    _server.send(200, "text/html", buildSuccessPage());
    _shouldReboot = true;
}

void ConfigWebServer::handleSaveProxy() {
    if (!_settings) {
        _server.send(500, "text/plain", "Internal error");
        return;
    }
    ...
    if (proxyUrl.length() == 0) {
        _server.send(400, "text/plain", "Proxy URL is required");
        return;
    }

    _settings->setProxyUrl(proxyUrl);
    Serial.printf("Proxy URL saved: %s\n", proxyUrl.c_str());
    _server.send(200, "text/html", buildSuccessPage());
    _shouldReboot = true;
}
...
void ConfigWebServer::handleScreenshot() {
    ...
    // No sprite buffer on direct-to-display boards (C6 AMOLED)
    _server.send(503, "text/plain", "Screenshot not available on this board (no sprite buffer)");
#else
...
String ConfigWebServer::buildPage() {
    ...
    bool hasProxy = _settings && _settings->hasProxy();
    ...
    page += hasKey  ? F("<div class='step-line done'></div>")
                    : F("<div class='step-line'></div>");
    // Dot 3: Proxy
    page += hasProxy ? F("<div class='step-dot done'>&#10003;</div>")
                     : (hasKey ? F("<div class='step-dot current'>3</div>")
                               : F("<div class='step-dot'>3</div>"));
    page += hasProxy ? F("<div class='step-line done'></div>")
                     : F("<div class='step-line'></div>");
    // Dot 4: Session
    page += hasSess ? F("<div class='step-dot done'>&#10003;</div>")
                    : (hasProxy ? F("<div class='step-dot current'>4</div>")
                                : F("<div class='step-dot'>4</div>"));
    page += F("</div>");
    ...
    page += hasProxy ? F("<span class='step-label done'>Proxy</span>")
                     : (hasKey ? F("<span class='step-label current'>Proxy</span>")
                               : F("<span class='step-label'>Proxy</span>"));
    page += hasSess  ? F("<span class='step-label done'>Session</span>")
                     : (hasProxy ? F("<span class='step-label current'>Session</span>")
    ...
    // ── Step 3: Cloud Proxy ──
    bool proxyOpen = hasKey && !hasProxy;
    page += "<div id='c3' class='card" + String(hasProxy ? " done" : (hasKey ? " active" : "")) +
            String(proxyOpen ? " open" : "") + "'>";
    page += F("<div class='card-head' onclick='toggleCard(\"c3\")'>");
    page += F("<div class='num'>3</div>");
    page += F("<div class='title'>Cloud Proxy</div>");
    if (hasProxy) {
        page += F("<span class='badge badge-ok'>Configured</span>");
    } else {
        page += F("<span class='badge badge-pending'>Required</span>");
    ...
    page += F("<div class='hint' style='margin-bottom:14px'>"
              "ESP32 can't connect to claude.ai directly. Deploy the included proxy to Vercel (free) to relay requests.</div>");

    page += F("<details style='margin-bottom:12px'>"
              "<summary>How to deploy</summary>"
              "<div style='padding:10px 0;font-size:12px;color:#8b949e;line-height:1.7'>"
              "<b style='color:#e6edf3'>1.</b> Create a free <a href='https://vercel.com' style='color:#ff9944'>Vercel</a> account<br>"
              "<b style='color:#e6edf3'>2.</b> Install Vercel CLI: <code style='color:#e6edf3'>npm i -g vercel</code><br>"
              "<b style='color:#e6edf3'>3.</b> In the <code style='color:#e6edf3'>cloud-proxy/</code> folder, run: <code style='color:#e6edf3'>vercel --prod</code><br>"
              "<b style='color:#e6edf3'>4.</b> Copy the deployment URL and paste below"
              "</div></details>");

    page += F("<form method='POST' action='/save-proxy'>");
    ...
    page += "<input type='text' id='proxyurl' name='proxyurl' placeholder='https://your-project.vercel.app' value='" + proxyUrl + "'>";
    page += F("<button type='submit' class='btn btn-sm'>Save Proxy &amp; Restart</button>");
    page += F("</form>");
    page += F("</div></div>");

    // ── Step 4: Claude.ai Session ──
    bool sessOpen = hasProxy && !hasSess;
    page += "<div id='c4' class='card" + String(hasSess ? " done" : (hasProxy ? " active" : "")) +
            String(sessOpen ? " open" : "") + "'>";
    page += F("<div class='card-head' onclick='toggleCard(\"c4\")'>");
    page += F("<div class='num'>4</div>");
    ...
    page += F("<div class='hint' style='margin-bottom:14px'>"
              "Your claude.ai session cookie — used to fetch subscription rate limits.</div>");
