...
void UIWidgets::drawLcarsFrame(GfxCanvas& spr, const char* title,
    ...
    // ========== TOP-LEFT ELBOW ==========
    // Solid filled block — no rounded inside corner.
    spr.fillRect(0, 0, SW + R, TH + R, CLR_AMBER);

    // Continuous AMBER bar from elbow to CLAUDE.AI pill (full TH height)
    spr.fillRect(SW + R, 0, SEG2_X - (SW + R), TH, CLR_AMBER);

    // Title text — vertically centered in bar
#if SCR_H >= 300
    smoothTextBg(spr, title, SW + R + 4, TH / 2,
                 LCARS_22, CLR_BG, CLR_AMBER, ML_DATUM);
#else
    smoothTextBg(spr, title, SW + R + 4, TH / 2,
                 LCARS_MD, CLR_BG, CLR_AMBER, ML_DATUM);
#endif

    // CLAUDE.AI pill — right segment of top bar, full TH height
    {
        const int16_t seg2RightX = SCR_W - RIGHT_INSET;
        const int16_t seg2R = TH / 2;
        spr.fillRoundRect(SEG2_X, 0, seg2RightX - SEG2_X, TH, seg2R, CLR_PEACH);
        spr.fillRect(SEG2_X, 0, seg2R, TH, CLR_PEACH);  // square left edge
#if SCR_H >= 300
        smoothTextBg(spr, "CLAUDE.AI", seg2RightX - 6, TH / 2,
                     LCARS_20, CLR_BG, CLR_PEACH, MR_DATUM);
#else
        smoothTextBg(spr, "CLAUDE.AI", seg2RightX - 6, TH / 2,
    ...
    // Scan line animation (small boards only)
#if SCR_H < 300
    {
        const int16_t gapEnd = SEG2_X - 4;
        const int16_t barX = SW + R + 4;
        const int16_t scanW = gapEnd - barX;
        if (scanW > 0) {
            float phase = (float)(millis() % 3000) / 3000.0f;
            int16_t scanX = barX + (int16_t)(phase * scanW);
            spr.fillRect(scanX, TH - 3, 3, 2, CLR_AMBER);
        }
    }
#endif
    ...
    // ========== BOTTOM-LEFT ELBOW ==========
    // Solid filled block — mirrors top-left elbow.
#if SCR_H >= 300
    {
        const int16_t botBarY = SCR_H - BOTTOM_INSET - BH;
        const int16_t botBarH = BH;       // full height

        // Solid block elbow (sidebar + corner)
        spr.fillRect(0, botBarY - R, SW + R, BH + R, CLR_TAN);

        // Continuous TAN bar across the full bottom
        const int16_t botBarRightX = SCR_W - RIGHT_INSET;
        spr.fillRect(SW + R, botBarY, botBarRightX - (SW + R), botBarH, CLR_TAN);

        // -- Page chip (rounded pill) --
        const int16_t bb_pageX = CONTENT_X;
        const int16_t bb_pageW = 44;  // wider pill for thick bar
        char pageBuf[8];
        snprintf(pageBuf, sizeof(pageBuf), "%d/%d", currentPage + 1, totalPages);
        spr.fillRoundRect(bb_pageX, botBarY, bb_pageW, botBarH, botBarH / 2, FRAME_PAGE_COLOR);
        spr.fillRect(bb_pageX, botBarY, botBarH / 2, botBarH, FRAME_PAGE_COLOR);
        smoothTextBg(spr, pageBuf, bb_pageX + bb_pageW / 2, botBarY + botBarH / 2,
                     LCARS_14, CLR_BG, FRAME_PAGE_COLOR, MC_DATUM);

        // -- Signal label + bars (on TAN background) --
        const int16_t bb_sigTextX = bb_pageX + bb_pageW + 8;
        smoothTextBg(spr, "SIGNAL", bb_sigTextX, botBarY + botBarH / 2,
                     LCARS_14, CLR_BG, CLR_TAN, ML_DATUM);
        drawSignalBars(spr, bb_sigTextX + 50, botBarY + (botBarH - 18) / 2, wifiRssi);

        // -- Countdown pill (AMBER, extends to right edge) --
        const int16_t bb_cdX = bb_sigTextX + 78;
        const int16_t bb_cdW = botBarRightX - bb_cdX;
        const int16_t cr = botBarH / 2;

        spr.fillRoundRect(bb_cdX, botBarY, bb_cdW, botBarH, cr, FRAME_COUNTDOWN_COLOR);
        spr.fillRect(bb_cdX, botBarY, cr, botBarH, FRAME_COUNTDOWN_COLOR);

        // "REFRESH IN" label
        smoothTextBg(spr, "REFRESH IN", bb_cdX + 6, botBarY + botBarH / 2,
                     LCARS_14, CLR_BG, FRAME_COUNTDOWN_COLOR, ML_DATUM);

        // Segmented progress bar
        {
            const int16_t pbX = bb_cdX + 68;
            const int16_t pbH = botBarH - 8;
            const int16_t pbY = botBarY + 4;
            const int16_t pbEnd = bb_cdX + bb_cdW - cr - 1;
            const int16_t segW = 4;
            const int16_t gap = 1;
            ...
            int nSegs = 0;
            for (int16_t tx = pbX; tx + segW <= pbEnd; tx += segW + gap) nSegs++;
            int filledSegs = (int)(nSegs * pct + 0.5f);
            int16_t sx = pbX;
            int seg = 0;
            while (sx + segW <= pbEnd) {
                uint16_t segColor = (seg < filledSegs) ? CLR_PEACH : CLR_BG;
                spr.fillRect(sx, pbY, segW, pbH, segColor);
                sx += segW + gap;
                seg++;
            }
        }
    ...
    char pageBuf[8];
    snprintf(pageBuf, sizeof(pageBuf), "%d/%d", currentPage + 1, totalPages);
    spr.fillRoundRect(FRAME_PAGE_X, barY, FRAME_PAGE_W, barH, barH / 2, FRAME_PAGE_COLOR);
    spr.fillRect(FRAME_PAGE_X, barY, barH / 2, barH, FRAME_PAGE_COLOR);
    spr.setTextDatum(MC_DATUM);
    spr.setTextColor(CLR_BG, FRAME_PAGE_COLOR);
    spr.drawString(pageBuf, FRAME_PAGE_X + FRAME_PAGE_W / 2, barY + barH / 2, 1);

    // Signal bars + label
    drawSignalBars(spr, FRAME_SIGNAL_BARS_X, barY + 1, wifiRssi);
    spr.setTextDatum(MR_DATUM);
    spr.setTextColor(FRAME_SIGNAL_TEXT_COLOR, CLR_BG);
    spr.drawString(FRAME_SIGNAL_TEXT_TEXT, FRAME_SIGNAL_TEXT_X, barY + barH / 2, 1);

    // Countdown pill with LCARS segmented progress bar
    {
        const int16_t cx = FRAME_COUNTDOWN_X;
        const int16_t cy = barY;
        const int16_t cw = FRAME_COUNTDOWN_W;
        const int16_t ch = barH;
        const int16_t cr = ch / 2;

        spr.fillRoundRect(cx, cy, cw, ch, cr, FRAME_COUNTDOWN_COLOR);
        spr.fillRect(cx, cy, cr, ch, FRAME_COUNTDOWN_COLOR);
        ...
        const int16_t barEnd = cx + cw - cr - 1;
        const int16_t segW = 4;
        const int16_t gap = 1;
        const uint32_t totalSec = REFRESH_INTERVAL_MS / 1000;
        float pct = (totalSec > 0 && countdownSec <= totalSec)
